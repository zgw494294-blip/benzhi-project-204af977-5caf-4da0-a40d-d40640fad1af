...
function renderActionPanel(session, samples, measurements) {
  ...
  if (session.status === 'measuring' || session.status === 'rework') {
    const next = samples.find((sample) => !measurements.some((measurement) => measurement.sampleID === sample.id && measurement.withinTolerance)) || samples[0];
